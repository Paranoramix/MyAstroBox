/**
 * C TCP Server socket.
 * This server can handle multiple clients using thread.
 * This server has a dedicated thread to read multiple sensors data.
 * Sensors are:
 *      - TX20 LaCrosse Technology (anemometer & wind direction)
 *      - DHT22 (temperature & relative humidity)
 *      - MLX90614 (infrared temperature).
 * 
 * Data is read from sensor about each 5 seconds and stored in variable.
 * TCP Server allow to request data sensor latest value or the 1000 lastest values.
 *
 * Compile
...
#define MLX90614_IR_TEMP_REG 7    // Register for Infrared temperature.
#define MLX90614_AM_TEMP_REG 6    // Register for ambiant temperature.

// Definition constants for DHT22 sensor
#define DHT22_MAXTIMINGS 85   // Max length for 1 bit
...
void enfiler(File *file, float tx20_ws, float tx20_wd, long tx20_lu, double mlx90614_at, double mlx90614_it, long mlx90614_lu, float dht22_at, float dht22_ru, long dht22_lu);
char* defiler(File *file, bool first);
...
int main(int argc , char *argv[]) {
  ...
  // Check if the program is started as sudoer.
  if (setuid(getuid()) < 0) {
...
//      , mlx90614_ambiant_temperature, mlx90614_infrared_temperature, (micros() - mlx90614_last_update), (unsigned) time(NULL));
    //  printf("%s %d\n", response, strlen(response));
			response = "{\"COMMAND\": \"GET_ALL_DATA_SENSORS\", \"VALUES\": [";
			write(sock, response, strlen(response));
			response =  defiler(myFile, true);
			while (response != NULL) {
				write(sock, response, strlen(response));
				free(response);
				response =  defiler(myFile, false);
			} 
			response = "]}";
			write(sock, response, strlen(response));
    }
...
void *read_sensors(void *args) {
  ...
  while(1) {
    ...
    //, tm.tm_year, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, tx20_wind_direction, tx20_wind_speed, mlx90614_infrared_temperature, mlx90614_ambiant_temperature, dht22_temperature, dht22_humidity);
		...
		delay(r * 1000 - 2000);
		enfiler(myFile, tx20_wind_speed, tx20_wind_direction, micros() - tx20_last_update, mlx90614_ambiant_temperature, mlx90614_infrared_temperature, micros() - mlx90614_last_update, dht22_temperature, dht22_humidity, micros() - dht22_last_update);
  }
...
void tx20_interrupt (void) {
  ...
  if (!is_tx20_interrupt) {
    ...
    // We do a little break to check bit at the half of his length.
...
void enfiler(File *file, float tx20_ws, float tx20_wd, long tx20_lu, double mlx90614_at, double mlx90614_it, long mlx90614_lu, float dht22_at, float dht22_ru, long dht22_lu) {
  ...
  nouveau->tx20_ws = tx20_ws;
  nouveau->tx20_wd = tx20_wd;
  nouveau->tx20_lu = tx20_lu;
  nouveau->mlx90614_at = mlx90614_at;
  nouveau->mlx90614_it = mlx90614_it;
  nouveau->mlx90614_lu = mlx90614_lu;
  nouveau->dht22_at = dht22_at;
  nouveau->dht22_ru = dht22_ru;
  nouveau->dht22_lu = dht22_lu;
	nouveau->ts = (unsigned) time(NULL);
...
char* defiler(File *file, bool first)
{
    ...
    if (file->premier != NULL)
    {
        Element *elementDefile = file->premier;
			if (first) {
      sprintf(response, "{ \"TX20_WIND_SPEED\": \"%.1f\", \"TX20_WIND_DIRECTION\": \" %.1f\", \"TX20_LAST_UPDATE\": \"%ld\", \"MLX90614_AMBIANT\": \"%.1f\", \"MLX90614_INFRARED\": \"%.1f\", \"MLX90614_LAST_UPDATE\": \"%ld\", \"DHT22_TEMPERATURE\": \"%.1f\", \"DHT22_HUMIDITY\": \"%.1f\", \"DHT22_LAST_UPDATE\": \"%ld\", \"SERVER_TIMESTAMP\": \"%u\" }"
      , elementDefile->tx20_ws
			, elementDefile->tx20_wd
			, elementDefile->tx20_lu
      , elementDefile->mlx90614_at
			, elementDefile->mlx90614_it
			, elementDefile->mlx90614_lu
      , elementDefile->dht22_at
			, elementDefile->dht22_ru
			, elementDefile->dht22_lu
			, elementDefile->ts);
			first = false;
			} else {
      sprintf(response, ", { \"TX20_WIND_SPEED\": \"%.1f\", \"TX20_WIND_DIRECTION\": \" %.1f\", \"TX20_LAST_UPDATE\": \"%ld\", \"MLX90614_AMBIANT\": \"%.1f\", \"MLX90614_INFRARED\": \"%.1f\", \"MLX90614_LAST_UPDATE\": \"%ld\", \"DHT22_TEMPERATURE\": \"%.1f\", \"DHT22_HUMIDITY\": \"%.1f\", \"DHT22_LAST_UPDATE\": \"%ld\", \"SERVER_TIMESTAMP\": \"%u\" }"
      ...
      , elementDefile->dht22_at
        ...
        free(elementDefile);
				result = malloc((strlen(response) + 1) * sizeof(char));
